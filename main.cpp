...
// Kalkulisanje faktora za korištenje u upscaling-u dobijenog bounding box-a
static int widthFactor = 640 / 96;
static int heightFactor = 480 / 96;
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; ix++) {
            ...
            bb.x *= widthFactor;
            bb.y *= heightFactor;
            bb.width *= widthFactor;
            bb.height *= heightFactor; 
            //offset
            bb.x += 40;
            bb.y += 20;
        //Draw bounding boxes    
	cv::rectangle(frame, cv::Point(bb.x, bb.y), cv::Point(bb.x+bb.width, bb.y+bb.height), cv::Scalar(255,255,255), 2);
                cv::putText(frame, bb.label, cv::Point(bb.x, bb.y-5), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0,0,0), 1);
